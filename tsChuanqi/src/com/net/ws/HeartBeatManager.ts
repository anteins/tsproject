module EIGame{
    export class HeartBeatManager extends EIGame.EISingleton{
        ...
        init(){
            this.checkHeartBeat();
            this.ping_packet = ProtobufHelper.Instance().encodeMsg(1004, {
                ping:1,
            }); 
        }
        ...
        checkHeartBeat() {
            ...
            this.sendCellFunc = setTimeout(()=> {
                if(!NetWork.Instance().connected() || !NetWork.Instance().IsOnline())
                    return;
        ...
        sendPingPacket() {
            if(this.ping_packet){
                NetWork.Instance().sendPacket(1004, this.ping_packet);
            }
        };
