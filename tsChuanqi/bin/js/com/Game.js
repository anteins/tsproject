...
(function (EIGame) {
    ...
    var Game = (function () {
        ...
        Game.prototype.init = function () {
            ...
            this.sp.loadImage(EIGame.ResManager.Instance().Path("logo.png"));
            EIGame.ViewManager.Instance().rootScene.addChild(this.sp);
            this.initMainSceneBtn();
        };
        Game.prototype.initMainSceneBtn = function () {
            var btn_w = 90;
            var btn_h = 20;
            var button_res = EIGame.ResManager.Instance().Path("threeDimen/ui/button.png");
            var self = this;
                ...
                if (EIGame.GameManager.mode == "2d") {
                    buttons = [
                        { "key": "性能测试 UI", "value": function () {
                                if (!EIGame.GameManager.Instance().isGamePlaying())
                                    return;
                                EIGame.ViewManager.Instance().openView("View_TEST2D");
                            } },
                        { "key": "登录 UI", "value": function () {
                                EIGame.ViewManager.Instance().openView("View_Login");
                            } },
                        { "key": "sample UI", "value": function () {
                                if (!EIGame.GameManager.Instance().isGamePlaying())
                                    return;
                                EIGame.ViewManager.Instance().openView("View_Samples");
                            } },
                        { "key": "清空", "value": function () {
                                if (!EIGame.GameManager.Instance().isGamePlaying())
                                    return;
                                EIGame.ViewManager.Instance().removeAllView();
                            } },
                        ...
                        { "key": "截屏", "value": function () {
                                ...
                                //此处将canvas指定区域进行截屏
                                var htmlC = EIGame.ViewManager.Instance().rootScene.drawToCanvas(640, 400, 0, 0);
                                //获取截屏区域的texture
                                ...
                                // sp2.scale(0.3, 0.3);
                                EIGame.ViewManager.Instance().rootScene.addChild(sp2);
                            } },
                    ];
                }
        ...
        Game.prototype.addButtons = function (buttons, button_res, start_x, start_y, btn_w, btn_h) {
            ...
            for (var i in buttons) {
                var kv = buttons[i];
                var btn = EIGame.ViewManager.Instance().rootScene.addChild(new Laya.Button(button_res, kv.key));
